...
class GraphAnalysis:
        ...
        for txt_file in input_files:
            with open(txt_file, 'r') as file:
                ...
                for line_idx in range(2, len(lines)):
                    line_data = lines[line_idx].split(", ")
                    data.append([line_data[2], int(line_data[0]), int(line_data[1])])
                ...
                elif "datasets_simulation" in txt_file:
                    self.dataset_simulation = data
                else:
    ...
    def create_charts(self, cites_upper_threshold):
        most_cited_boundingbox = self._create_chart_template(self.boundingbox_content, cites_upper_threshold, 'Bounding_box')
        ...
        most_cited_dataset_simulation = self._create_chart_template(self.dataset_simulation, cites_upper_threshold, 'Datasets_simulation')
        ...
        print('Most cited datasets simulation:', most_cited_dataset_simulation)
    ...
    def _create_chart_template(self, content_list, cites_upper_threshold, figure_name):
        ...
        for index, x in enumerate(ordered_list):
            algorithm = x[0][:15]  # restricting str length
            citations = x[2]            
        ...
        # Puts x ticks only for red bars for depth (too extense?)
        # red_bar_x_ticks = [x[0][:15] for counter, x in enumerate(ordered_list) if counter >= lower_percentile_index]
        ...
        plt.legend((red, gray),
                   ('≥{} cites'.format(most_cited_algorithms[0][2]), '≤{} cites'.format(ordered_list[lower_percentile_index-1][2])),
                   loc=4)
...
if __name__ == "__main__":
    ...
    , 'articles_datasets_real_world.txt', 'articles_depth_estimation_reduced.txt', "articles_datasets_simulation.txt"]
